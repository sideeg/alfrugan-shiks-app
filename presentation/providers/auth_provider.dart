...
import '../providers/usecase_providers.dart'; // new import
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  void forceLogout() {
    state = const AuthState(); // reset to unauthenticated
  }
  ...
  Future<void> login(String email, String password) async {
    ...
    result.fold(
      ...
      (loginResponse) {
        // ✅ Now we get the full response!
        state = state.copyWith(
          isLoading: false,
          isAuthenticated: true,
          user: loginResponse.user,
          minimumRequiredVersion:
              loginResponse.minimumRequiredVersion, // ✅ SET IT!
          errorMessage: null,
        );
      },
    );
  ...
  Future<void> logout() async {
    state = state.copyWith(isLoading: true, errorMessage: null);
    try {
      await logoutUseCase(NoParams()); // remote call
      await getIt<AuthLocalDataSource>().clearAuthData();
      state = const AuthState(); // reset
    } catch (_) {
      // even if remote fails we clear local data
      await getIt<AuthLocalDataSource>().clearAuthData();
